...
function enableMusicAutoplay() {
  ...
  window.removeEventListener('pointerdown', enableMusicAutoplay);
}
window.addEventListener('pointerdown', enableMusicAutoplay);
...
class Toy extends WorldObject {
  ...
  collectToy(toy) {
    ...
    // Remove the toy from the elements.toys array
    const idx = elements.toys.indexOf(toy)
    if (idx !== -1) elements.toys.splice(idx, 1)
